 ...
 */
  ...
  public:
    ...
    BandsFactory(const Config &config, bool verbose=false);
    ~BandsFactory();
    ...
    void ReprojBands(std::vector<Band> &BandArray, std::map<TString, std::vector<int> >  &InfoReprojArray, std::vector<Band> &ReprojArray,TString configname);
